...
void AnimationManager::PlayAnim() {
  ...
  while (active_anim != active_anims.end()) {
    ...
    for (auto& blend_anims : anim_mesh->active_animations) {
      ...
      for (int i = 0; i < anim_mesh->armature->num_bones; i++) {
        auto root_bone = &anim_mesh->armature->bones[i];
        if (root_bone->parent == nullptr) {
          SetTransformForHierarchy(anim_mesh->armature, root_bone, blend_anims.second, root_bone_parent, final_transforms);
        }
      }
    }
    mesh_data_buffer->BufferBoneTransform(final_transforms);
    active_anim++;
  }
}
...
mat4 AnimationManager::_GetBoneAnimation(const Bone* bone, const vector<ActiveAnimationBlend>& blend_anims, bool& out_is_bone_anim) {
...
Animation* AnimationManager::CreateAnimation(string& anim_name, double duration, double ticks_per_seconds) {
  Animation* anim = new Animation{ anim_id_pool->ObtainNumber(), anim_name, duration, ticks_per_seconds, {}};
  anims.push_back(anim);
  anims_num_of_active_instances[anim->id] = 0;
  return anim;
}
ActiveAnimation* AnimationManager::CreateActiveAnimInstance(Armature* armature) {
  auto anim_mesh = new ActiveAnimation(armature, active_anim_id_pool->ObtainNumber());
  all_anim_meshes.push_back(anim_mesh);
  return anim_mesh;
}
void AnimationManager::LoadAnimationForArmature(
  ...
  if (armature == nullptr) {
    LOG((ostringstream()
      << "' armature. Skip LoadAnimationForMesh for file: "
      << file_name)
        .str());
    return;
  }
  ...
  for (size_t i = 0; i < scene->mNumAnimations; i++)
  {
    ...
    armature->name_to_anim[anim_name] = anim;
  }
}
