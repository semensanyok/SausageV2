...
public:
  ...
  static void RenderGui(SDL_Window *window, Camera *camera) {
...

    {
      ...
      ImGui::Begin("Camera");
      ImGui::InputFloat3("POSITION", &camera->pos[0], "%.0f",
                         ImGuiInputTextFlags_None);
      ImGui::Text("Yaw: %.1f Pitch: %.1f", camera->yaw_angle,
                  camera->pitch_angle);
      ImGui::Text("Right: %.1f %.1f %.1f", camera->right.x, camera->right.y,
                  camera->right.z);
      ImGui::Text("Up: %.1f %.1f %.1f", camera->up.x, camera->up.y,
                  camera->up.z);
      ImGui::Text("direction: %.1f %.1f %.1f", camera->direction.x,
    ...
    }
    {
      ...
      ImGui::Begin("Physics");
      ImGui::InputFloat("physics step multiplier",
                        &GameSettings::physics_step_multiplier, 0.001f, 1.0f,
                        "%.6f");
      ImGui::InputInt("ray_debug_draw_lifetime_milliseconds",
                      &GameSettings::ray_debug_draw_lifetime_milliseconds, 500);
...
#ifdef SAUSAGE_PROFILE_ENABLE
    {
      ...
      if (is_update_time) {
        ...
        t2 =
            std::chrono::duration_cast<time_unit>(ProfTime::render_commands_ns);
        t3 =
            std::chrono::duration_cast<time_unit>(ProfTime::render_commands_ns);
        ...
        t6 = std::chrono::duration_cast<time_unit>(
            ProfTime::render_swap_window_ns);
        ...
        t9 = std::chrono::duration_cast<time_unit>(
            ProfTime::physics_buf_trans_ns);
        t10 = std::chrono::duration_cast<time_unit>(
            ProfTime::physics_debug_draw_world_ns);
      ...
      if (ImGui::TreeNode("Profile")) {
        ...
        ImGui::Checkbox("Debug Draw Physics",
                        &StateManager::GetInstance()->phys_debug_draw);
        ImGui::TreePop();
      }
