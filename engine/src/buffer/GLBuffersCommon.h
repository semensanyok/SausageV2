#ifndef GL_BUFFERS_COMMON_H
#define GL_BUFFERS_COMMON_H
...
// COMPOSITE FLAGS
const BufferTypeFlag MESH_BUFFERS = MESH_UNIFORMS | TEXTURE | LIGHT;
...
const BufferTypeFlag OUTLINE_BUFFERS = 0;
const BufferTypeFlag UI_BUFFERS = TEXTURE | UI_UNIFORMS | CONTROLLER_UNIFORMS;
}; // namespace BufferType
...
BufferSlots<T> *CreateBufferSlots(unsigned long storage_size,
                                  ...
                                  GLuint array_type, ArenaSlotSize slot_size) {
  GLuint buffer_id;
  ...
  // storage_size, flags);
  T *buffer_ptr = MapGetBuferSlotsPtr<T>(buffer_id, storage_size, array_type);
  return new BufferSlots<T>{
      {Arena({0, max_arena_slots, 0}, slot_size)}, buffer_id, buffer_ptr};
}

template <typename T>
T *MapGetBuferSlotsPtr(GLuint buffer_id, unsigned long storage_size,
                       GLuint array_type) {
  glBindBuffer(array_type, buffer_id);
  return (T *)glMapBufferRange(array_type, 0, storage_size, flags);
}
...
BufferNumberPool<T> *CreateBufferNumberPool(unsigned long storage_size,
  ...
  glBindBuffer(array_type, buffer_id);
  glBufferStorage(array_type, storage_size, NULL, flags);
  T *buffer_ptr = (T *)glMapBufferRange(array_type, 0, storage_size, flags);
  return new BufferNumberPool<T>{ThreadSafeNumberPool(max_number), buffer_id,
                                 buffer_ptr};
}

#endif // GL_BUFFERS_COMMON_H
